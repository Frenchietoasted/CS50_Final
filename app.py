    ...
    if request.method == 'POST':
        ...
        for post in subreddit.hot(limit=100):
            if stock in post.title.lower() and i<4:
                ...
                redcomments.append(post.num_comments)
        ...
        db.execute("INSERT INTO stocks (company,sentiment) VALUES (?,?)",(stock,sentiment)) 
        conn.commit()             
        return render_template('search.html',redurls=redurls,redcomments=redcomments,redtitles=redtitles,stock=stock,redscore=red_score,news=news['articles'][:5],newsscore=newsscores,i=i)
    else:
        return render_template('Apology.html',apology='GET request not allowed') 